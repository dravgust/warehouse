...
namespace Vayosoft.Threading.Channels.Producers
{
    public abstract class AsyncProducerConsumerChannelBase<T>
    {
        ...
        protected AsyncProducerConsumerChannelBase(string channelName, uint startedNumberOfWorkerThreads = 1,
            bool enableTaskManagement = false, CancellationToken globalCancellationToken = default)
        {
            ...
            {
                var w = new ConsumerAsync<T>(_channel, ConsumerName, OnDataReceived, _cancellationToken);
            ...
            try
            {
                ...
                else if (workersDiff < 0) // missing workers, need to add
                {
                    ...
                    for (var i = 0; i < workersDiff; i++)
                    {
                        ...
                        var w = new ConsumerAsync<T>(_channel.Reader, ConsumerName, OnDataReceived, _cancellationToken);
                        _workers.Add(w);
                        w.StartConsume();
                        processedWorkers++;
                    }
